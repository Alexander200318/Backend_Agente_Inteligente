...
def obtener_por_usuario_agente(self, id_usuario: int, id_agente: int):
    ...
    """
    return self.repository.get_permisos_usuario_agente(id_usuario, id_agente)
...
def actualizar_por_usuario_agente(self, id_usuario: int, id_agente: int, data: UsuarioAgenteUpdate):
    ...
    asignacion = self.repository.get_by_usuario_agente(id_usuario, id_agente)
    
    if not asignacion:
        return None
    
    # Actualizar usando el ID de la asignación
    return self.repository.update(asignacion.id_usuario_agente, data)